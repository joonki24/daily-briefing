...
`radarImageUrl`(https)이 옵니다. 이미지를 보려면 단축어 5번 뒤에 다음을 추가하세요.

1. **"사전에서 값 가져오기"** → 키 `radarImageUrl` (비 예보가 없으면 비어 있음)
2. **"조건"**: 위 결과가 **값이 있음**이면 → **"URL 콘텐츠 가져오기"**(GET, 헤더 없음) → **"이미지 변환"** → **"빠른 보기"**

레이더 이미지가 안 올 때는 서버 로그의 `[radarImage]` 경고와 `/logs`의 `note: 레이더 이미지 없음`을 확인합니다.
...
- **예외: 화~토 브리핑 밑의 "📌 시장 이슈"(2~3줄)만 Claude를 씁니다.** 연합뉴스 증시·국제 / 한국경제 증권 RSS에서
...
- ⚠️ **개인/가족용으로만 쓰세요.** 연합뉴스 기사 하단에 "무단 전재·재배포, AI 학습 및 활용 금지" 문구가 있습니다.
  이 서비스는 하루 한 번 기사 1~2개만 읽고, 본문은 저장·전달하지 않으며, 결과에는 출처 없이 짧은 요약만 싣지만,
