...
파이프라인 3개(증시/뉴스/외출준비) 재작성은 전부 끝났습니다. 남은 건 실제 키/환경으로
검증하는 것과 운영 편의 항목들입니다.

- [ ] `.env` 값 채우기 (위 1번 표)
- [ ] `curl -X POST .../test/morning`로 `config.json`의 `stock.indices` 심볼이 Twelve Data와
      맞는지 실제 키로 확인 (안 맞으면 심볼만 수정)
- [ ] `curl "localhost:3000/depart?place=..."`로 경로/날씨 응답과 부분 응답(`partial: true`)
      동작 확인
- [ ] MBC/KBS/채널A/JTBC 스크레이핑이 실제로 되는지 확인 (샌드박스에서는 네트워크가 막혀
      있어 못 해봄)
