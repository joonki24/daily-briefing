...
> `NTFY_TOPIC`은 선택(비우면 푸시 없음). **진행 현황(2026-09-24)**: ① Oracle 배포 완료(도쿄, E2.1.Micro 1GB + 스왑, pm2 자동시작,
> IP `150.230.211.135`) ② 외부 주소는 Cloudflare Tunnel 대신 **sslip.io + Caddy**로 완료
> (`https://150-230-211-135.sslip.io`, 도메인 구매 불필요, 서버가 클라우드라 Tunnel이 필요 없었음).
...
- **구조화된 숫자/필드 데이터(지수, 좌표, 날씨 수치 등)는 LLM으로 "요약"하지 않는다.** (단 화~토 증시 브리핑의
  "시장 이슈"는 뉴스 본문 요약이라 예외로 Claude를 쓴다 — 개인/가족용 전제, 실패해도 지수는 그대로 나감)
